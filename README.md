...
让本机智能体安全、自动地把文章上传到微信公众号草稿箱的服务端 API Bridge。把它部署到一台具有固定公网出口 IP 的服务器，并将该 IP 加入公众号开发平台的白名单后，豆包、WorkBuddy、Codex 等运行在自己电脑上的智能体便可通过统一 API 创建草稿、上传素材、查询状态；它们不需要直接点击公众号网页后台。

> 本仓库不包含任何生产服务器信息、真实域名、公众号标识、API Key、AppSecret、回调 Token、管理员凭据、数据库、日志或运行截图。

## 解决什么问题

公众号运营往往依赖网页后台操作；但当网页操作受到反自动化机制、登录态或运行环境限制时，本机智能体无法可靠地通过“点击网页”完成上传草稿。与此同时，微信公众号开放接口通常要求把调用方的固定出口 IP 加入白名单，个人电脑与多个智能体的网络环境很难稳定满足这一条件。

本项目把需要固定 IP 的微信 API 调用集中到一台受控服务器：

1. 在公众号开发平台将该服务器的固定出口 IP 加入白名单；
2. 将公众号 `AppID`、`AppSecret` 留在服务器，而不是发给本地智能体；
3. 本机智能体通过 HTTPS 调用 Bridge，由 Bridge 代表它们上传素材、创建草稿或查询发布状态；
4. 为不同智能体和不同公众号分别签发 API Key，实现“多个智能体运营多个公众号”，同时保留权限隔离与审计记录。

因此，Bridge 不是绕过公众号权限体系的浏览器自动化工具；它是在已获得账号授权、IP 白名单许可的前提下，使用官方 API 为智能体提供受控接入层。
...
本项目采用 [MIT License](LICENSE)。你可以使用、修改、发布和商用代码，但必须保留版权与许可证声明；软件按“现状”提供，不附带担保。
